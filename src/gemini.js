...
- The split is LABEL-DRIVEN, not amount-driven. If a VAT-class LABEL is printed in the totals block, emit a line_item for it even if the amount is small, ambiguous, unreadable, or appears to be zero. Do NOT silently drop a bucket because its amount looks tiny — the labels are the source of truth.
...
- If the printed labels exist but you cannot read the bucket amount confidently, set amount = 0 for that line and add a warning to warnings[] like "Zero-rated bucket label printed but amount unreadable".
- Only skip a bucket entirely when the LABEL is not present on the receipt at all. A bucket with label "ZERO-RATED SALES: 0.00" must still be emitted as a line with amount=0.
...
- BIR-receipt math sanity: if grand_total ≈ vatable_base × 1.12 EXACTLY (math closes cleanly to one vatable bucket) but the receipt visibly prints "Zero-Rated Sales" / "VAT-Exempt Sales" labels in the totals block, you have likely misread either the grand_total or the bucket amounts. Re-examine the totals block carefully and emit the labelled buckets.
    ...
    assignments: {
      ...
      items: {
        ...
        properties: {
          ...
          expense_category: { type: "string", description: "The expense category CONSISTENT WITH the account you chose: office_supplies|meals|repairs|rent|fuel|professional_fees|outsourced_services|advertising|freight|utilities|inventory|equipment|commission|contractor|other. This is the authoritative category — it overrides the extraction-time category for EWT determination." },
...
10. EWT IMPLICATIONS — your account choice IS the EWT determination. Choose the account that correctly reflects the nature of the payment, because the account drives the withholding tax rate:
   ...
   - Advertising / marketing / promotional spend → "Advertising and Promotion" account → expense_category: "advertising" (EWT: 2%, WC120)
...
11. EXPENSE_CATEGORY MUST MATCH ACCOUNT — for every line, set expense_category to the value that is consistent with the account you chose. The system uses expense_category to apply the correct withholding tax; an inconsistency between account and category causes wrong EWT. Examples:
   - You chose "628000 Advertising and Promotion" → expense_category must be "advertising" (EWT: 2%)
